...
def register(mcp: FastMCP) -> None:
    ...
    async def get_tags() -> dict:
        """List all tags connected to ALTR (available for
        use in policies).

        Only tags connected to ALTR via `connect_tag` appear
        here. Tags created
        in Snowflake but not yet connected will not be listed.
        """
    ...
    async def delete_tag(tag_group_id: str) -> dict:
        """Delete a tag from ALTR.

        All policies on the tag must be removed first,
    ...
    async def get_tag_details_by_group_id(tag_group_id: str) -> dict:
        """Get full details for a specific tag masking by its group ID.
    ...
    async def connect_tag(
        ...
        """Connect a Snowflake tag to ALTR so it can be
        used in masking policies.

        The tag must already exist in Snowflake. Once
        connected, it will appear in `get_tags`
        and can be used with `create_policy`.
