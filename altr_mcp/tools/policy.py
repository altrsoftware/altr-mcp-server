...
def register(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    @log_tool
    async def create_policy(tag: str) -> dict:
        """Create a masking policy for a specific tag.

        Creates an empty masking policy that controls how
        data tagged with the specified tag is masked. Until
        you add rules with `add_rules`, all users
        will see NULL for tagged columns.

        The tag must already be connected to ALTR (visible in `get_tags`). Each
        tag can only have one policy — check `get_policies` first to avoid
        conflicts.
        ...
        """
        settings = get_settings()
        response = await policy.create_altr_policy({}, settings.auth, tag)
