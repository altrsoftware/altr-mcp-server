...
Manage database connections in ALTR. Connect new data sources (Snowflake, Databricks, OLTP), update connection properties, check status, and remove databases.

## Tool Summary

| Tool | Description |
|------|-------------|
| `get_databases` | List all databases connected to ALTR |
| `get_database_id` | Get the numeric ALTR database ID for a database name |
| `get_service_users` | List service users available for connections |
...
Discover which databases are connected to ALTR (Snowflake, Databricks, OLTP). Returns connection metadata including database names and IDs. Use `get_database_id` to get the numeric ID required for classification jobs.
...
List service users available for database connections. Returns service user IDs needed for `create_database` (Snowflake keypair auth) and `create_databricks_database` (Databricks token auth).
...
Connect a new Snowflake or OLTP data source to the ALTR platform. For Databricks, use `create_databricks_database` instead. Supports two authentication modes:
...
Connect a Databricks workspace to the ALTR platform. Supports two authentication modes:
...
After creation, use `get_databases` to confirm the connection. The `database_id` returned is required for `create_databricks_job`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `friendly_database_name` | str | yes | Display name for the database in ALTR |
| `database_name` | str | yes | Databricks catalog or workspace name |
| `hostname` | str | yes | Databricks workspace URL (e.g. `"https://adb-1234567890.azuredatabricks.net"`) |
| `service_user_id` | str | no | Service user ID from `get_service_users` for token auth |
| `database_username` | str | no | Username (password auth only) |
| `database_password` | str | no | Password or personal access token (password auth only) |
