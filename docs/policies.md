...
Create a masking policy for a specific tag. Creates an empty policy -- until you add rules with `add_rules`, all users will see NULL for tagged columns.

The tag must already be connected to ALTR (visible in `get_tags`). Each tag can only have one policy.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `tag` | str | yes | Tag name (UPPERCASE) as returned by `get_tags` |
