...
Create and manage access management policies that control which roles can access which databases, schemas, or tables. Supports both Snowflake and OLTP datasources.
...
Trigger a manual compliance check for a grant/access management policy. Runs the policy check immediately instead of waiting for the next scheduled run.
